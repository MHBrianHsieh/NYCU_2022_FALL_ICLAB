...
// -----------------------------------------------------------------------------
// Revision History
// VERSION Date       AUTHOR           DESCRIPTION                   AREA            CYCLE  POWER (pattern_num / wocg / cg / reduction %)
// 1.0     2022-11-17 Brian Hsieh                                    115093.441467   0/1/2  100 / 8.007e-03 / 5.664e-03 / 29.26%
// 2.0     2022-11-20 Brian Hsieh      reduce operations to 9 bits   115093.441467   0/1/2  100 / 8.007e-03 / 5.664e-03 / 29.26%
// 3.0     2022-11-20 Brian Hsieh      omit sleep_ctrl in submodules 108437.314682   0/1/2  100 / 7.572e-03 / 5.600e-03 / 26.04%
//                                                                                          1000_mode5 / 7.430e-03 / 6.021e-03 / 18.96%
// 3.1     2022-11-20 Brian Hsieh      revise input of submodules    107762.055763   0/1/2  1000 / 7.273e-03 / 5.262e-03 / 27.65%
//                                     & revise gray2bin
// 3.2     2022-11-20 Brian Hsieh      omit input_sleep_ctrl         107555.818957   0/1/2  1000 / 7.620e-03 / 5.213e-03 / 31.59%
// 4.0     2022-11-20 Brian Hsieh      revise maxmin in addsub       86253.553136    0/1/2  1000 / 5.662e-03 / 3.478e-03 / 38.57%
// 4.1     2022-11-20 Brian Hsieh      gated comp_sleep_ctrl         86313.428329    0/1/2  1000 / 5.718e-03 / 3.519e-03 / 38.46%
// 4.2     2022-11-20 Brian Hsieh      revise comp_sleep_ctrl        87001.993132    0/1/2  1000 / 5.745e-03 / 3.470e-03 / 39.60%
// 5.0     2022-11-20 Brian Hsieh      revise find_sleep_ctrl        85551.682742    0/1/2  1000 / 5.496e-03 / 3.232e-03 / 41.19%
// 5.1     2022-11-20 Brian Hsieh      revise sleep_ctrl             85960.829895    0/1/2  1000 / 5.612e-03 / 3.324e-03 / 40.77%
// 5.2     2022-11-20 Brian Hsieh      revise find_sleep_ctrl        85698.044324    0/1/2  1000 / 5.457e-03 / 3.193e-03 / 41.49%
// 5.3     2022-11-20 Brian Hsieh      revise input of submodules    85561.661948    0/1/2  1000 / 5.504e-03 / 3.280e-03 / 40.41%
// 6.0     2022-11-20 Brian Hsieh      gated sma_in(comb)            85721.329070    0/1/2  1000 / 4.678e-03 / 2.357e-03 / 49.62%
// 6.1     2022-11-20 Brian Hsieh      revise out_data (tt)          86177.045945    0/1/2  1000 / 4.688e-03 / 2.365e-03
// 7.0     2022-11-20 Brian Hsieh                                    85714.676315    0/1/2  1000 / 5.548e-03 / 2.377e-03 / 57.16%
// 8.0     2022-11-20 Brian Hsieh                                    85654.801126    0/1/2  1000 / 5.683e-03 / 1.991e-03 / 64.97%
// 9.0     2022-11-20 Brian Hsieh                                    85874.343513    0/1/2  1000 / 5.699e-03 / 1.987e-03 / 65.13%
// 10.0    2022-11-20 Brian Hsieh      share the same reg array      77069.362831    0/1/2  1000 / 4.499e-03 / 2.188e-03 / 51.37%
// -----------------------------------------------------------------------------
// KEYWORDS: General file searching keywords, leave bank if none.
//
// -----------------------------------------------------------------------------
// PURPOSE: Short description of functionality
// Series Processing (with clock gating)
// -----------------------------------------------------------------------------
// PARAMETERS
...
// Instantiations : GATED_OR (clock gating)
// Other : 
// -----------------------------------------------------------------------------
...
// gated clock sleep control
// reg                input_sleep_ctrl;
reg                comp_sleep_ctrl;
// reg                addsub_sleep_ctrl;
reg                sma_sleep_ctrl;
reg                find_sleep_ctrl;
// reg                out_sleep_ctrl;
...
// always @(*) begin
//   if (!cg_en) begin
//     input_sleep_ctrl = 1'b0;
//   end
//   else begin
//     if ( (current_state != IDLE) || (current_state != GRAY)) begin
//       input_sleep_ctrl = 1'b0;
//     end
//     else begin
//       input_sleep_ctrl = 1'b1;
//     end
//   end
// end
...
always @(*) begin
  if (!cg_en) begin
    ...
    if ( in_mode_reg_cs[1] ) begin
      ...
      comp_sleep_ctrl = 1'b1;
    end
  end
end
// always @(*) begin
//   if (cg_en && !in_mode_reg_cs[1]) begin
//     comp_sleep_ctrl = 1'b1;
//   end
//   else begin
//     comp_sleep_ctrl = 1'b0;
...
// always @(*) begin
//   if (!cg_en) begin
//     addsub_sleep_ctrl = 1'b0;
//   end
//   else begin
//     if (current_state != ADD_SUB) begin
//       addsub_sleep_ctrl = 1'b1;
//     end
//     else begin
//       addsub_sleep_ctrl = 1'b0;
//     end
...
// end
// always @(*) begin
//   if (cg_en && (current_state != ADD_SUB)) begin
//     addsub_sleep_ctrl = 1'b1;
//   end
//   else begin
//     addsub_sleep_ctrl = 1'b0;
...
// always @(*) begin
//   if (cg_en && (current_state != SMA)) begin
//     sma_sleep_ctrl = 1'b1;
//   end
//   else begin
//     sma_sleep_ctrl = 1'b0;
...
always @(*) begin
  if (!cg_en) begin
    find_sleep_ctrl = 1'b0;
  end
  else begin
    if ( !current_state[2] ) begin
      find_sleep_ctrl = 1'b1;
    end
    else begin
      find_sleep_ctrl = 1'b0;
    end
...
// always @(*) begin
//   if (cg_en && !current_state[2]) begin
//     find_sleep_ctrl = 1'b1;
//   end
//   else begin
//     find_sleep_ctrl = 1'b0;
...
always @(posedge comp_clk or negedge rst_n) begin
  if (!rst_n) begin
...
//     always @(posedge addsub_clk or negedge rst_n) begin
//       if (!rst_n) begin
...
generate
  for (n=0; n<9; n=n+1) begin: sma_out_dff
    always @(posedge sma_clk or negedge rst_n) begin
      if (!rst_n) begin
        sma_out_cs[n] <= 9'd0;
      end
      else begin
...
// median
always @(posedge find_clk or negedge rst_n) begin
  if (!rst_n) begin
    median_cs <= 9'd0;
  end
  else begin
...
//   .CLOCK       (clk               ),
//   .SLEEP_CTRL  (input_sleep_ctrl  ),
//   .RST_N       (rst_n             ),
//   .CLOCK_GATED (input_clk         )
// );


GATED_OR GATED_comp (
  .CLOCK       (clk               ),
  .SLEEP_CTRL  (comp_sleep_ctrl   ),
  .RST_N       (rst_n             ),
  .CLOCK_GATED (comp_clk          )
);


// GATED_OR GATED_addsub (
//   .CLOCK       (clk               ),
//   .SLEEP_CTRL  (addsub_sleep_ctrl ),
//   .RST_N       (rst_n             ),
//   .CLOCK_GATED (addsub_clk        )
// );


GATED_OR GATED_sma (
  .CLOCK       (clk               ),
  .SLEEP_CTRL  (sma_sleep_ctrl    ),
  .RST_N       (rst_n             ),
  .CLOCK_GATED (sma_clk           )
);
...
GATED_OR GATED_find (
  .CLOCK       (clk               ),
  .SLEEP_CTRL  (find_sleep_ctrl   ),
  .RST_N       (rst_n             ),
  .CLOCK_GATED (find_clk          )
);


// GATED_OR GATED_out (
//   .CLOCK       (clk               ),
//   .SLEEP_CTRL  (out_sleep_ctrl    ),
//   .RST_N       (rst_n             ),
//   .CLOCK_GATED (out_clk           )
// );
